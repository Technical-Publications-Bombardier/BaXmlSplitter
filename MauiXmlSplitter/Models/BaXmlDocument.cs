...
public class BaXmlDocument : XmlDocument
{
    ...
    public XmlNode[]? SelectNodesByCheckout(string xpath, string[] checkoutNames)
    {
        ...
        for (var i = 0; i < nodes.Count; i++)
        {
            ...
            if (checkoutNames.Contains(node.Name, StringComparer.OrdinalIgnoreCase)) selectedNodes.Add(node);
        }
        ...
        for (var i = 0; i < nodes.Count; i++)
        {
            ...
            while (ancestor.Name is { } ancestorName &&
                   !checkoutNames.Contains(ancestorName, StringComparer.OrdinalIgnoreCase) &&
                   ancestor.ParentNode is { } greatAncestor) ancestor = greatAncestor;
